...
public class MH36 extends MangaParser {
    ...
    public MH36(Source source) {
        init(source, new MH36.Category());
    }

    @Override
    public Request getSearchRequest(String keyword, int page) {
        String url = StringUtils.format("http://m.36mh.com/search/?keywords=%s&p=%d", keyword, page);
    ...
    public SearchIterator getSearchIterator(String html, int page) {
        ...
        for (Node node : body.list("#update_list > ul > li > a")) {
            try {
        ...
        return new NodeIterator(body.list("#update_list  div.itemBox")) {
            @Override
            protected Comic parse(Node node) {
                String cid = node.hrefWithSplit("div.itemTxt a:eq(0)", 1);
                String title = node.text("div.itemTxt a:eq(0)");
                String cover = node.attr("div.itemImg mip-img", "src");
                String update = node.text("a.coll");
                String author = node.text("div.itemTxt p");
    ...
    public Request getInfoRequest(String cid) {
        String url = "http://m.36mh.com/manhua/".concat(cid);
        return new Request.Builder().url(url).build();
    }
    ...
    public void parseInfo(String html, Comic comic) {
        Node body = new Node(html);
        String title = body.text("div.view-sub.autoHeight > h1");
        String cover = body.src("#Cover > mip-img");
        String update = body.text(" div.view-sub.autoHeight > div > dl:nth-child(2) > dd");
        String author = body.text("div.view-sub.autoHeight > div > dl:nth-child(3) > dd");
    ...
    public List<Request> getImagesRequest(String cid, String path) {
        ...
        try {
            String url = StringUtils.format("http://m.36mh.com/manhua/%s/%s.html", cid, path);
            response = App.getHttpClient().newCall(new Request.Builder().url(url).build()).execute();
            Integer page = Integer.parseInt(new Node(response.body().string()).text("#k_total"));
            for (int i = 1; i <= page; ++i) {
                url = StringUtils.format("http://m.36mh.com/manhua/%s/%s-%d.html", cid, path, i);
    ...
    public List<ImageUrl> parseImages(String html) {
        ...
        try {
            String src = new Node(html).src("div.UnderPage > div:nth-child(3) mip-img");
            list.add(new ImageUrl(1, src, false));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return list;
    ...
    private static class Category extends MangaCategory {
        ...
        public String getFormat(String... args) {
            return StringUtils.format("http://www.36mh.com/list/area-%s-smid-%s-year-%s-lz-%s-order-%s-p-%%d",
    ...
    public Headers getHeader() {
        return Headers.of("Referer", "http://m.36mh.com/");
    }
}
